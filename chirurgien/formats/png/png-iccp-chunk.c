                ...
                gint        *chunk_counts)
{
    ...
    format_utils_add_text_tab (&tab, "ICC profile name", profile_name, utf8_length);

    if (!profile_name_length)
        format_utils_add_line_no_section_tab (&tab, "NOTE: No profile name defined, profile name should be at least 1 byte long");
    else if (profile_name_length >= 80)
        format_utils_add_line_no_section_tab (&tab, "NOTE: The profile name exceeds its 79 bytes long limit");
    ...
    if (compression_method != -1)
    {
        ...
        format_utils_add_line_tab (&tab, "Compression method", compression_method_value,
                                   "ICC profile compression method\n"
                ...
                if (flags_found)
                {
                    ...
                    if (decompression_success)
                    {
                        format_utils_add_field (file, CHUNK_DATA_COLOR_2, TRUE, deflate_size,
                                                "ZLIB compressed ICC profile", NULL);
    ...
    format_utils_add_line_no_section_tab (&tab, "NOTE: ICC profile names are encoded using ISO-8859-1");
