...
gif_logical_screen_descriptor_block (FormatsFile *file)
{
    ...
    if (!process_gif_field (file, NULL, "Logical screen width", NULL,
                            "Maximum value: 2<sup>16</sup> - 1 (unsigned 16-bit integer)",
                            BLOCK_DATA_COLOR_1, 2, "%u", NULL))
        return FALSE;

    /* Logical screen height */
    if (!process_gif_field (file, NULL, "Logical screen height", NULL,
                            "Maximum value: 2<sup>16</sup> - 1 (unsigned 16-bit integer)",
    ...
    if (!process_gif_field (file, NULL, "Background color index", NULL, NULL,
                            BLOCK_DATA_COLOR_2, 1, NULL, &fields[1]))
        return FALSE;

    /* Pixel aspect ratio */
    if (!process_gif_field (file, NULL, "Pixel aspect ratio", NULL, NULL,
                            BLOCK_DATA_COLOR_1, 1, NULL, &fields[2]))
        return FALSE;

    /* Color resolution */
    value = g_strdup_printf ("%u bits", ((fields[0] >> 4) & 0x7) + 1);
    format_utils_add_line (file, "Color resolution", value,
    ...
    if (fields[2])
    {
        ...
        format_utils_add_line (file, "Aspect ratio", value,
                               "Aspect ratio aproximation, calculated as:"
    ...
    if (fields[0] >> 7)
    {
        format_utils_start_section (file, "Global Color Table");

        if ((fields[0] >> 3) & 0x1)
            value = "Yes";
        else
            value = "No";
        format_utils_add_line (file, "Sorted", value,
        ...
        format_utils_add_line (file, "Background color index", value,
                               "Global Color Table index of the background color");
        g_free (value);

        global_color_table_size = 1 << ((fields[0] & 0x7) + 1);

        value = g_strdup_printf ("%u", global_color_table_size);
        format_utils_add_line (file, "Global Color Table entries", value,
        ...
        for (gint i = 0; i < global_color_table_size; i++)
        {
            if (i % 2)
                format_utils_add_field (file, BLOCK_DATA_COLOR_1, TRUE, 3,
                                        "Global Color Table entry (RGB triplet)", NULL);
            else
