                         ...
                         AnalyzerTab *exif_ascii_tab)
{
    ...
    if (FILE_HAS_DATA_N (file, 2))
    {
        *tagged_bytes = g_slist_append (*tagged_bytes, GUINT_TO_POINTER (GET_POINTER (file)));
        ...
        analyzer_utils_tag_navigation (file, IFD_COLOR, 2, _("Number of directory entries (Exif)"), "Exif IFD");
    }
    else
    {
       return;
    ...
    while (ifd_entries)
    {
        ...
        else if (tiff_tag == tiff_tags[RelatedSoundFile])
        {
            process_ascii_tag (file, exif_ascii_tab, _("Tag: RelatedSoundFile"), "RelatedSoundFile", field_type,
                               count, 13, value_offset, is_little_endian, tagged_bytes);
        }
        ...
        else if (tiff_tag == tiff_tags[ImageUniqueID])
        {
            process_ascii_tag (file, exif_ascii_tab, _("Tag: ImageUniqueID"), "ImageUniqueID", field_type,
                               count, 33, value_offset, is_little_endian, tagged_bytes);
        }
